...
for j in {1..5}
do
  curl -g "https://librairy.linkeddata.es/solr/covid-paragraphs/select?q=bionlp_drugs_C$j:[*%20TO%20*]%20AND%20bionlp_drugs_N$j:[*%20TO%20*]&wt=csv&indent=true&start=0&rows=50000000&fl=id,bionlp_drugs_C$j,bionlp_drugs_N$j" > paragraphs-bionlp_drugs_C$j.csv
done
...
for j in {1..19}
do
  echo "Normalizing columns bionlp_diseases_$j in paragraphs file"
  python3 normalization-twocolumns.py -f paragraphs-bionlp_diseases_C$j.csv -c bionlp_diseases_C$j -c2 bionlp_diseases_N$j -s ,
done
